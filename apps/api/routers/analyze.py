...
class LeakInfo(BaseModel):
    ...
    mistake_rate: float
    ev_loss: float
    common_mistakes: dict[str, int] | None = None
    ...
    if worst_positions and worst_positions[0][1].mistake_rate > 10:
        ...
        position_advice = {
            "BB": (
                "Big Blind Defense",
                "大盲防守",
                "You're losing significant EV in BB. Focus on defending wider against late position steals, especially from BTN and CO.",
                "你在大盲位置損失了大量 EV。專注於對後位偷盲進行更寬的防守，尤其是對按鈕位和切位。",
            ),
            "SB": (
                "Small Blind Play",
                "小盲玩法",
                "Your SB leaks suggest either over-folding or poor 3-betting frequencies. Consider a polarized 3-bet or fold strategy.",
                "你的小盲漏洞顯示可能過度棄牌或 3-bet 頻率不佳。考慮使用極化的 3-bet 或棄牌策略。",
            ),
            "BTN": (
                "Button Play",
                "按鈕位玩法",
                "Button should be your most profitable position. Review your opening ranges and consider stealing more aggressively.",
                "按鈕位應該是你最賺錢的位置。檢視你的開池範圍，考慮更積極地偷盲。",
            ),
            "CO": (
                "Cutoff Play",
                "切位玩法",
                "Cutoff mistakes often come from not opening wide enough or poor 3-bet defense. Review your CO ranges.",
                "切位錯誤通常來自開池不夠寬或 3-bet 防守不佳。檢視你的切位範圍。",
            ),
            "HJ": (
                "Hijack Play",
                "HJ位玩法",
                "HJ requires balanced opening ranges. You may be opening too wide or too tight for this position.",
                "HJ位需要平衡的開池範圍。你可能在這個位置開池過寬或過緊。",
            ),
            "UTG": (
                "Under The Gun Play",
                "UTG位玩法",
                "UTG requires the tightest ranges. If you're making mistakes here, you may be opening too wide.",
                "UTG需要最緊的範圍。如果你在這裡犯錯，可能是開池過寬。",
            ),
        }
        ...
        title, title_zh, desc, desc_zh = position_advice.get(
            ...
            (
                f"{pos} Play",
                f"{pos}位玩法",
                f"Focus on improving your {pos} play.",
                f"專注於改善你的{pos}位玩法。",
            ),
        )
        ...
        if "vs_rfi" in scenario:
            if "BB" in scenario:
                ...
                insights.append(
                    AIInsight(
                        ...
                        title_zh="練習：面對開池",
                        description="Your response to raises needs work. Practice the VS-RFI drill to improve.",
                        description_zh="你對開池的應對需要加強。練習 VS-RFI 訓練來改善。",
                        priority=2,
                        drill_link="/drill/vs-rfi",
                    )
                )
            ...
            insights.append(
                AIInsight(
                    ...
                    description="Your 3-bet play has leaks. Practice responding to 3-bets with the VS-3bet drill.",
                    description_zh="你的 3-bet 玩法有漏洞。使用 VS-3bet 訓練來練習應對 3-bet。",
                    priority=2,
...
def generate_overall_assessment(
    mistake_rate: float, total_ev_loss: float, skill_level: str, top_leaks: list[LeakInfo]
) -> tuple[str, str]:
...
def get_ai_review(request: Request, data: AIReviewRequest):
    ...
    insights = generate_ai_insights(
        data.position_stats, data.top_leaks, data.mistake_rate, data.total_ev_loss
    )

    # Generate overall assessment
    overall_en, overall_zh = generate_overall_assessment(
        data.mistake_rate, data.total_ev_loss, skill_level, data.top_leaks
    )
