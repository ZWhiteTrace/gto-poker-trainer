...
const LOGIC_QUESTIONS = [
  // Range Construction
  {
    ...
    options: [
      {
        key: "a",
        text: "BTN 後面只有兩個位置要行動，被 3bet 的機率較低",
        textEn: "Only 2 positions left to act, lower 3bet probability",
      },
    ...
    correctAnswer: "a",
    explanation:
      "位置越靠後，後面要行動的人越少，被加注的機率越低。BTN 只需要通過 SB 和 BB 兩個位置，而 UTG 後面有 5 個位置可能加注。",
    explanationEn:
      "The later the position, fewer players left to act means lower chance of facing a raise. BTN only needs to get through SB and BB, while UTG has 5 positions that could raise.",
  },
  {
    id: "range_2",
    category: "range_construction",
    ...
    options: [
      ...
      {
        key: "b",
        text: "因為這些手牌處於 EV 接近的邊界",
        textEn: "Because these hands are at the EV indifference boundary",
      },
    ...
    correctAnswer: "b",
    explanation:
      "處於邊界的手牌（如某些 Axs），加注和跟注的 EV 非常接近。為了保持範圍平衡且不可被剝削，這些手牌需要以一定頻率分配到不同行動。",
    explanationEn:
      "Borderline hands (like certain Axs) have very similar EV for raising and calling. To maintain balanced, unexploitable ranges, these hands must be distributed across actions at certain frequencies.",
  },
  // Position Advantage
  {
    id: "position_1",
    ...
    options: [
      {
        key: "a",
        text: "可以看到對手先行動再做決定",
        textEn: "Can see opponent's action before deciding",
      },
    ...
    correctAnswer: "a",
    explanation:
      "有位置意味著你在每條街都最後行動，可以根據對手的行動獲得額外信息後再做決定。這讓你可以更精準地控制底池大小、更有效地榨取價值或詐唬。",
    explanationEn:
      "Being in position means acting last on every street, gaining information from opponent's actions before deciding. This allows better pot control, more efficient value extraction, and effective bluffing.",
  },
  {
    id: "position_2",
    category: "position_advantage",
    ...
    options: [
      {
        key: "a",
        text: "因為 BB 已經投入盲注，需要更寬地防守來保護盲注",
        textEn: "BB already has chips invested and needs to defend wider",
      },
    ...
    correctAnswer: "a",
    explanation:
      "BB 已經投入 1BB，面對 2.5BB 加注只需要再投入 1.5BB。底池賠率使得 BB 可以有利可圖地跟注更多邊緣手牌，否則會被 BTN 過度剝削。",
    explanationEn:
      "BB already invested 1BB, facing 2.5BB raise only needs to add 1.5BB. Pot odds make it profitable for BB to call with more marginal hands, otherwise BTN would over-exploit.",
  },
  // Bet Sizing
  {
    id: "sizing_1",
    ...
    options: [
      {
        key: "a",
        text: "因為你有範圍優勢，小注即可達成目標",
        textEn: "You have range advantage, small bet achieves goals",
      },
    ...
    correctAnswer: "a",
    explanation:
      "乾燥 A 高牌面（如 A72r）對開牌者非常有利：你範圍中的 Ax 比例高於 BB。BB 的很多手牌在這種牌面很難繼續。小注可以有效施壓，同時保護你的弱牌範圍。",
    explanationEn:
      "Dry ace-high boards (like A72r) heavily favor the raiser: higher Ax proportion than BB. Many BB hands struggle to continue. Small bets apply pressure while protecting weaker hands in your range.",
  },
  {
    id: "sizing_2",
    category: "bet_sizing",
    ...
    options: [
      {
        key: "a",
        text: "因為極化範圍的價值牌需要更大保護，詐唬牌需要更大 fold equity",
        textEn: "Value hands need protection, bluffs need more fold equity",
      },
    ...
    correctAnswer: "a",
    explanation:
      "極化策略意味著你的範圍只有超強牌和詐唬。大注讓堅果牌獲得最大價值，同時給詐唬足夠的 fold equity。中等牌力的手牌會選擇較小的尺寸。",
    explanationEn:
      "Polarized strategy means your range only contains nuts and bluffs. Large bets maximize value from nut hands while giving bluffs sufficient fold equity. Medium strength hands prefer smaller sizes.",
  },
  // Frequency Balance
  {
    id: "frequency_1",
    ...
    options: [
      {
        key: "a",
        text: "為了讓對手的 bluff-catch 手牌變成零 EV",
        textEn: "To make opponent's bluff-catchers zero EV",
      },
      { key: "b", text: "因為詐唬很刺激", textEn: "Because bluffing is exciting" },
      {
        key: "c",
        text: "因為有時候沒有好牌",
        textEn: "Because sometimes you don't have good cards",
      },
      { key: "d", text: "為了贏更多", textEn: "To win more" },
    ],
    correctAnswer: "a",
    explanation:
      "如果你河牌從不詐唬，對手可以輕鬆棄掉所有非堅果牌。GTO 要求足夠的詐唬頻率使對手的 bluff-catch 變成零 EV，讓他們無法有利可圖地過度跟注或過度棄牌。",
    explanationEn:
      "If you never bluff the river, opponents can easily fold all non-nut hands. GTO requires sufficient bluff frequency to make opponent's bluff-catches zero EV, preventing profitable over-calling or over-folding.",
  },
  {
    id: "frequency_2",
    category: "frequency_balance",
    ...
    options: [
      {
        key: "a",
        text: "面對下注時需要繼續的最低頻率，防止對手有利可圖地詐唬",
        textEn: "Minimum continue frequency vs bet to prevent profitable bluffing",
      },
    ...
    correctAnswer: "a",
    explanation:
      "MDF = 1 - (Bet / (Pot + Bet))。例如面對底池大小的下注，MDF = 50%。如果你棄牌頻率高於 50%，對手可以有利可圖地用任何兩張牌詐唬。",
    explanationEn:
      "MDF = 1 - (Bet / (Pot + Bet)). For example, vs pot-sized bet, MDF = 50%. If you fold more than 50%, opponent can profitably bluff any two cards.",
  },
  // ICM Concepts
  {
    id: "icm_1",
    ...
    options: [
      {
        key: "a",
        text: "因為出局的代價（失去所有籌碼價值）大於加倍籌碼的收益",
        textEn: "Cost of busting (losing all chip value) > benefit of doubling",
      },
    ...
    correctAnswer: "a",
    explanation:
      "ICM 的核心是籌碼價值非線性。在泡沫期，即將進錢圈，出局意味著失去所有已累積的獎金期望值。而贏得籌碼的邊際價值遞減，使得風險回報比不利於冒險。",
    explanationEn:
      "ICM's core is non-linear chip value. On the bubble, about to cash, busting means losing all accumulated prize expectation. Diminishing marginal value of winning chips makes risk-reward unfavorable.",
  },
];
            ...
            {question.options.map((option) => {
              ...
              return (
                <Button
                  ...
                  className={cn(
                    "h-auto w-full justify-start px-4 py-4 text-left",
