...
async function waitForScenario(page: Page) {
  ...
  await page.locator('text="Your Hand"').first().waitFor({ state: "visible", timeout: 15000 });
}
...
test.describe("Multistreet Drill", () => {
  ...
  test("selecting action shows street result", async ({ page }) => {
    ...
    await expect(
      page
        .locator(
          "text=/Correct|正確|could be better|可以更好|Check-Raise|Villain|對手|Deal|Next|deal|next/i"
        )
        .first()
  ...
  test("complete full 3-street hand", async ({ page }) => {
    ...
    // === HAND REVIEW ===
    await expect(page.locator("text=/Review|回顧|Hand Review|手牌回顧/i").first()).toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(
      page
        .locator("button")
        .filter({ hasText: /next|下一手|Next/i })
        .first()
    ).toBeVisible();
  });
