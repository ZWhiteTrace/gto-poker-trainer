...
test.describe("Push/Fold Drill", () => {
  ...
  test("generates spot and shows hand + position + stack", async ({ page }) => {
    ...
    await expect(page.locator("text=/Position/i").first()).toBeVisible();
  });
  ...
  test("answering push/fold shows result", async ({ page }) => {
    ...
    await expect(page.locator("text=/Correct|Incorrect|正確|錯誤/i").first()).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("next hand button loads new spot after answering", async ({ page }) => {
    ...
    if (await nextBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      await expect(
        page
          .locator("button")
          .filter({ hasText: /^(Push|Fold|Call|Shove)$/i })
          .first()
